...
{
    public $expired_date_from;
    public $expired_date_to;
    /**
     * @inheritdoc
     */
    ...
    {
        return [
            [['id', 'quantity', 'customer_id', 'coupon_type_id', 'quantity_used', 'partner_id', 'promotion_type', 'updated_at', 'created_by', 'updated_by', 'commission_for'], 'integer'],
            [['title', 'slug', 'coupon_code', 'expired_date', 'description', 'created_at'], 'safe'],
            [['promotion_value'], 'number'],
            [['expired_date_from','expired_date_to'], 'safe'],
        ];
    }
...
//    }
    ...
    public function search($params)
    {
        ...
        $query->andFilterWhere([
            ...
            self::tableName() . '.commission_for' => $this->commission_for,
        ...
        if ($this->expired_date_from) {
            $query->andWhere([
                '>=',
                'expired_date',
                date_format(date_create($this->expired_date_from),"Y-m-d"),
            ]);
        }
    
        if ($this->expired_date_to) {
            $query->andWhere([
                '<=',
                'expired_date',
                date_format(date_create($this->expired_date_to),"Y-m-d"),
            ]);
        }
